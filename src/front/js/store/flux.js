	...
	return {
		...
		actions: {
			...
			handleCreateProduct: async (data) => {
				...
				if (!response.ok)
					return toast.error("hubo un error con la creacion del producto");
			...
			getUserProducts: async () => {
				...
				try {
					const response = await fetch(
						`${process.env.BACKEND_URL}/api/user-products`,
						{
							headers: {
			...
			handleTransaccion: async (product_id, transaccion_status) => {
				...
				const response = await fetch(
					`${process.env.BACKEND_URL}/api/transaccion`,
					{
						method: "POST",
						body: JSON.stringify({
							product_id: product_id,
							transaccion_status: transaccion_status,
						}),
						headers: {
							"Content-Type": "application/json",
			...
			deleteProduct: async (id) => {
				const response = await fetch(
					`${process.env.BACKEND_URL}/api/delete/${id}`,
					{
						method: "DELETE",
